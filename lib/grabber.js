...
class Grabber {
        ...
        while (idx <= data.finalChapter && data.running){
            ...
            this.fs.mkdir('./'+idx, (e) => {
                if(!e || (e && e.code === 'EEXIST')){
                    ...
                    this.grab_chapter(data, current_chapter)
                } else {
                    console.log(e)
                }
            })
    ...
    async grab_chapter(data, current_chapter){
        const PadNumber = require('./pad_number.js')
        var chapterData = {
            url : data.url,
        ...
        while (!chapterData.chapter_completed && data.running) {
            ...
            this.download(new_url, chapterData.page, chapterData, data)
    ...
    download(url, page, chapterData, data){
        ...
        module.get(url, (response) => {
            if (response.statusCode === 200) {
                ...
                const file = fs.createWriteStream('./'+chapterData.chapter+"/"+chapterData.page+".jpg")
                response.pipe(file)
            } else {
